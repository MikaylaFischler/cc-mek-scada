...
local function make(parent, x, y, unit)
    ...
    assert(num_boilers  >= 0 and num_boilers  <= 5, "minimum 0 boilers, maximum 5 boilers")
    assert(num_turbines >= 1 and num_turbines <= 6, "minimum 1 turbine, maximum 6 turbines")
    ...
    do
        ...
        if no_boilers then 
            table.insert(steam_pipes_b, pipe(0, 2, 3, 1+(i*8), colors.white))
            table.insert(steam_pipes_b, pipe(1, 3, 3, 2+(i*8), colors.blue)) 
        else

            table.insert(turbine_pipes, pipe(0, 0+((i-1)*8), 0, 0+((i-1)*8), colors.white))
    ...
    if not no_boilers then
        table.insert(steam_pipes_b, pipe(1, 8.999999, 1, 2, colors.white, false, false))
        table.insert(steam_pipes_b, pipe(1, 1, 3, 1, colors.white, false, false))

        table.insert(steam_pipes_b, pipe(2, 8.9999999, 2, 3, colors.blue, false, false))
