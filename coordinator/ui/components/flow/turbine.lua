...
local function make(parent, unit_id, close_cb)
    ...
    -- bounding box div
    local root = Div{parent=parent,x=math.floor((parent.get_width()-140)/2),y=2,width=140,height=50}
    ...
    local steam_div = Div{parent=window,x=2,y=1,width=22,height=8}
    ...
    local energy_div = Div{parent=window,x=25,y=1,width=24,height=8}
    ...
    local s_flow = Rectangle{parent=window,x=2,y=10,width=23,height=18,border=border(1,gray,true),thin=true,fg_bg=parent.get_fg_bg()}

    TextBox{parent=s_flow,text="Steam Flow",alignment=ALIGN.CENTER}

    TextBox{parent=s_flow,y=3,text="Vents",width=8,fg_bg=style.label}
    local vents = DataIndicator{parent=s_flow,format="%8d",value=0,commas=true,lu_colors=lu_c,width=8,fg_bg=s_field}
    vents.register(ps, "vents", vents.update)

    TextBox{parent=s_flow,x=10,y=3,text="Dispersers",width=12,fg_bg=style.label}
    local dispersers = DataIndicator{parent=s_flow,x=10,format="%12d",value=0,commas=true,lu_colors=lu_c,width=12,fg_bg=s_field}
    dispersers.register(ps, "dispersers", dispersers.update)

    TextBox{parent=s_flow,y=6,text="Maximum Flow Rate",width=21,fg_bg=style.label}
    local max_flow = DataIndicator{parent=s_flow,format="%16d",value=0,unit="mB/t",commas=true,lu_colors=lu_c,width=21,fg_bg=s_field}
    max_flow.register(ps, "max_flow_rate", max_flow.update)

    TextBox{parent=s_flow,y=9,text="Steam Input Rate",width=21,fg_bg=style.label}
    local input_rate = DataIndicator{parent=s_flow,format="%16d",value=0,unit="mB/t",commas=true,lu_colors=lu_c,width=21,fg_bg=s_field}
    input_rate.register(ps, "steam_input_rate", input_rate.update)

    TextBox{parent=s_flow,y=12,text="Steam Flow Rate",width=21,fg_bg=style.label}
    local flow_rate = DataIndicator{parent=s_flow,format="%16d",value=0,unit="mB/t",commas=true,lu_colors=lu_c,width=21,fg_bg=s_field}
    flow_rate.register(ps, "flow_rate", flow_rate.update)

    TextBox{parent=s_flow,y=15,text="Flow Utilization",width=21,fg_bg=style.label}
    local flow_bar = HorizontalBar{parent=s_flow,show_percent=true,bar_fg_bg=steam_c,height=1,width=21}
    flow_bar.register(ps, "flow_rate", function (v) flow_bar.update(v / (ps.get("max_flow_rate") or 1)) end)

    --#endregion
    --#region generator

    local e_flow = Rectangle{parent=window,x=26,y=10,width=23,height=18,border=border(1,gray,true),thin=true,fg_bg=parent.get_fg_bg()}
    ...
    prod_bar.register(ps, "prod_rate", function (v) prod_bar.update(v / (ps.get("max_production") or 1)) end)

    --#endregion
    --#region water flow

    local w_flow = Rectangle{parent=window,x=2,y=29,width=47,height=9,border=border(1,gray,true),thin=true,fg_bg=parent.get_fg_bg()}

    TextBox{parent=w_flow,text="Water Return",alignment=ALIGN.CENTER}

    TextBox{parent=w_flow,y=3,text="Condensers",width=21,fg_bg=style.label}
    local condensers = DataIndicator{parent=w_flow,format="%21d",value=0,commas=true,lu_colors=lu_c,width=21,fg_bg=s_field}
    condensers.register(ps, "condensers", condensers.update)

    TextBox{parent=w_flow,x=25,y=3,text="Max. Water Output",width=21,fg_bg=style.label}
    local max_water = DataIndicator{parent=w_flow,x=25,format="%16d",value=0,unit="mB/t",commas=true,lu_colors=lu_c,width=21,fg_bg=s_field}
    max_water.register(ps, "max_water_output", max_water.update)

    TextBox{parent=w_flow,x=25,y=6,text="Water Flow Rate",width=21,fg_bg=style.label}
    local water_ret = DataIndicator{parent=w_flow,x=25,format="%16d",value=0,unit="mB/t",commas=true,lu_colors=lu_c,width=21,fg_bg=s_field}
    water_ret.register(ps, "flow_rate", function (r) water_ret.update(math.min(r, ps.get("max_water_output"))) end)

    TextBox{parent=w_flow,y=6,text="Water Return Util.",width=21,fg_bg=style.label}
    local water_bar = HorizontalBar{parent=w_flow,show_percent=true,bar_fg_bg=water_c,height=1,width=21}
    water_bar.register(ps, "flow_rate", function (v) water_bar.update(v / (ps.get("max_water_output") or 1)) end)
